...
def main():
    ...
    csv_file1 = r'C:\Users\IT\Downloads\recon_updated (1)\Recon (2)\Recon\Output_Files\output_PayTM.csv'

    # Create a MySQL connection
    connection = create_connection()
    
    if connection:
        ### Delete from Table shifted to separate script
        delcursor = connection.cursor()
        # Delete Existing Data
        #delcursor.execute("DELETE FROM paytm_phonepe")
        #connection.commit()

        insert_data_from_csv(csv_file1, connection)
        connection.commit()
...
#         # Insert data into paytm_phonepe table
#         for i, row in df.iterrows():
#             sql = f"INSERT INTO paytm_phonepe ({', '.join(df.columns)}) VALUES ({', '.join(['%s'] * len(row))})"
#             cursor.execute(sql, tuple(row))
...
#         # Commit the transaction
#         connection.commit()
#         print(f"{cursor.rowcount} rows inserted successfully into paytm_phonepe table.")
...
# def main():
#     """Main function to load PayTM CSV and insert into MySQL"""
#     # Auto-detect CSV path
#     script_dir = Path(__file__).parent
#     csv_file_path = script_dir / 'Output_Files' / 'output_PayTM.csv'
    
#     print(f"Looking for PayTM CSV file at: {csv_file_path}")
    
#     if not csv_file_path.exists():
#         print(f"❌ PayTM CSV file not found at {csv_file_path}")
#         return
...
#             connection.commit()
#             print("✅ PayTM data loaded successfully")
